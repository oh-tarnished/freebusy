...
type rpcMetrics struct {
	Requests int64 `pulse:"metric:counter:organisation.rpc.requests"`
	Errors   int64 `pulse:"metric:counter:organisation.rpc.errors"`
}

// traced runs fn inside a pulse span named "OrganisationService/<method>",
// records request/error counters, and logs the outcome. Expected client/business
...
func traced(ctx context.Context, method string, fn func(context.Context) error) error {
	return shared.Pulse.Tracing.Trace(ctx, "OrganisationService/"+method, nil, func(ctx context.Context, _ *pulse.Span) error {
		err := fn(ctx)
