...
type rpcMetrics struct {
	Requests int64 `pulse:"metric:counter:property.rpc.requests"`
	Errors   int64 `pulse:"metric:counter:property.rpc.errors"`
}

// traced runs fn inside a pulse span named "PropertyService/<method>", records
// request/error counters, and logs the outcome. Expected client/business
// rejections are logged at debug, not error, and excluded from the error counter.
func traced(ctx context.Context, method string, fn func(context.Context) error) error {
	return shared.Pulse.Tracing.Trace(ctx, "PropertyService/"+method, nil, func(ctx context.Context, _ *pulse.Span) error {
		err := fn(ctx)
...
// outcome. A non-status error reads as Unknown, which counts.
...
// are already gRPC statuses pass through unchanged; anything else becomes Internal.
