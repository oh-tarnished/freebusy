...
type rpcMetrics struct {
	Requests int64 `pulse:"metric:counter:booking.rpc.requests"`
	Errors   int64 `pulse:"metric:counter:booking.rpc.errors"`
}

// traced runs fn inside a pulse span named "BookingService/<method>", records
// request/error counters, and logs the outcome. Expected client/business
// rejections are logged at debug and excluded from the error counter.
func traced(ctx context.Context, method string, fn func(context.Context) error) error {
	return shared.Pulse.Tracing.Trace(ctx, "BookingService/"+method, nil, func(ctx context.Context, _ *pulse.Span) error {
		err := fn(ctx)
...
// toStatusErr maps repository sentinel errors onto gRPC status codes. A capacity
// conflict surfaces as FailedPrecondition (the request can't be satisfied in the
// current state), distinct from an etag/state Aborted conflict.
