...
// matching the group itself ("policy") or any field beneath it ("policy.notes").
// An empty mask selects every group (replace-all).
...
// the stored record). Scalars are copied field-by-field; the nested address,
// policy, and media gallery are replaced wholesale when selected, so the
// repository can re-materialize each child from the merged proto. Identity,
// timestamps, state, units, and etag are managed by the repository.
...
// immutable and never merged. The pricing groups (price, rate_overrides,
// los_discounts, fees, taxes), media, and applicable_promo_codes are replaced
// wholesale when selected.
