...
// explains why and Discount/FinalTotal are nil.
type Result struct {
	Valid      bool
	Reason     string
...
func Evaluate(pc *promocodepbv1.PromoCode, subtotal *money.Money, resource, offering string, now time.Time) Result {
	if reason, ok := redeemable(pc, subtotal, resource, offering, now); !ok {
...
// redeemable reports whether pc may be redeemed for this request, returning a
// human-readable reason when it may not.
func redeemable(pc *promocodepbv1.PromoCode, subtotal *money.Money, resource, offering string, now time.Time) (string, bool) {
	if pc.GetDisabled() {
		return "promo code is disabled", false
	}
	if pc.GetState() == promocodepbv1.PromoCodeState_PROMO_CODE_STATE_EXPIRED {
		return "promo code has expired", false
	}
	if start := pc.GetRedeemStartTime(); start != nil && now.Before(start.AsTime()) {
		return "promo code is not yet redeemable", false
	}
	if end := pc.GetRedeemEndTime(); end != nil && now.After(end.AsTime()) {
		return "promo code has expired", false
	}
	if max := pc.GetMaxRedemptions(); max > 0 && pc.GetRedemptionCount() >= max {
		return "promo code redemption limit reached", false
	}
	...
	if min := pc.GetMinSubtotal(); min != nil {
		if !sameCurrency(min, subtotal) {
			return "promo code currency does not match the booking subtotal", false
		}
		if toNanos(subtotal) < toNanos(min) {
			return "subtotal is below the required minimum", false
		}
	}
	...
	if res := pc.GetApplicableResources(); len(res) > 0 && !contains(res, resource) {
		return "promo code is not applicable to this resource", false
	}
	if off := pc.GetApplicableOfferings(); len(off) > 0 && !contains(off, offering) {
		return "promo code is not applicable to this offering", false
	}
	return "", true
}
