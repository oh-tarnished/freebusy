...
// write persists the promo code together with its normalized child rows
// (discount, redemption window, usage limits, scope), their Money value-objects
// (common schema), and the scope's applicable-resources / -offerings join rows
// inside a single transaction; each read re-hydrates them via preloads.
